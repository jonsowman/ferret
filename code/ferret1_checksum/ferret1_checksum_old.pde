...
while (1){
 ...
 sprintf(Radiostring,"$$ORION,%lu,%02u:%02u:%02u,%d.%04u,%d.%04u,%d,%u",ticks,GPShr,GPSmin,GPSsec,latst,latend,lonst,lonend,alt,sats);
...
void SSerialwrite(int pin_no, char data, int highval, int lowval, int
baud, float stopbit)
{
byte mask;
...
void writestring(int pin_no,char *str, int highval, int lowval, int
baud, float stopbit)
{
int j=0;
